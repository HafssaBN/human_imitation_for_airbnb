...
def main():
    ...
    out = HostConfig.CONFIG_OUTPUT_FILE
    ...
    if hosts:
        print(f"📈 Average listings per host: {sum(_fetch_reviews_count(sqlite3.connect(HostConfig.CONFIG_DB_FILE), h[0]) * 0 + len(_fetch_listings_for_host(sqlite3.connect(HostConfig.CONFIG_DB_FILE), h[0])) for h in hosts)/len(hosts):.1f}")
